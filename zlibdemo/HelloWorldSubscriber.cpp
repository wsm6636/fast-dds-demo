        ...
        Subscriber* sub)
{
    ...
    if (sub->takeNextData((void*)&m_Hello, &m_info))
    {
        if (m_info.sampleKind == ALIVE)
        {
            ...
            // Print your structure data here.
           	if (outfile.is_open())
   		 {
				ulong tlen = m_Hello.index();
				uLong blen = compressBound(tlen);
				char* buffer = (char*)malloc(sizeof(char)*blen);
    				char* recdata = (char*)malloc(sizeof(char)*tlen);
				recdata = m_Hello.data().data();
				//ss << m_Hello.message();
				//ss >> s;
				//s = base64_decode(m_Hello.message());
				uncompress((Bytef *)buffer, &blen, (Bytef *)recdata, tlen); 
				std::cout << "uncompress size :"<< blen << std::endl;				
				s = buffer;				
				s = base64_decode(s);
				std::cout << "decode size : "<< s.length() << std::endl;
				const char *p = s.c_str();				
				outfile.write(p, blen);	
		...
		while (fgets(buf, sizeof(buf), fp)) {
			...
			pclose(fp);
			subresult = "result " + std::string(result);
	...
	std::cout << "with data" << m_Hello.data().data() << std::endl;
